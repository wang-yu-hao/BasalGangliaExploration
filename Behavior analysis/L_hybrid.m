function [L, Q, S] = L_hybrid(params, dt_subject)
...
%             / (exp(beta * q(stim1)) + exp(beta * q(stim2)));
            ...
            c(c==0) = .1;
            sigma = s ./ sqrt(c);
%             A = beta_1 * q / sqrt(sum(sigma.^2)) + beta_2 * sigma + beta_3 * q;

            if num_arms == 2
                A = beta_1 * q / sqrt(sum(sigma.^2)) + beta_2 * sigma;
%                 P = 1 / (1 + exp(-((2*A(choices(t)) - sum(A)))));
                P = normcdf(2*A(choices(t)) - sum(A));
            else
                A = beta_1 * q / sqrt(sigma(shown(t, 1))^2 + sigma(shown(t, 2))^2) + beta_2 * sigma;
