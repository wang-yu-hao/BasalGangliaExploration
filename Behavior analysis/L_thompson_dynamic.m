function [L, Q, S] = L_thompson_dynamic(params, dt_subject)
...
%                 P = 1 / (1 + exp(-(2*A(choices(t)) - sum(A))));
                ...
                A = (q) ./ sqrt(2 * e^2 + lambda ^2 * s(shown(t, 1))^2 * (a + b* (m + k * c(shown(t, 1))^power)) ^2 /N + s(shown(t, 2))^2 * (a + b* (m + k * c(shown(t, 2))^power)) ^2 /N);
