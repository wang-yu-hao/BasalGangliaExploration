function [L, Q, S] = L_fixed(alpha_q, alpha_s, lambda, e, m, k, a, b, power, N, dt_subject)
...
%                 P = 1 / (1 + exp(-(2*A(choices(t)) - sum(A))));
                ...
                A = (q + lambda * k * sigma + lambda * m * s) ./ sqrt(2 * e^2 + lambda ^2 * s(shown(t, 1))^2 * (a + b* (m + k * c(shown(t, 1))^power)) ^2 /N + s(shown(t, 2))^2 * (a + b* (m + k * c(shown(t, 2))^(-0.5))) ^2 /N);
